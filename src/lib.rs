...
pub extern "C" fn pxs_addfunc(module_ptr: *mut pxs_Module, name: *const c_char, func: pxs_Func) {
    ...
    if !module
        .callbacks
        .iter()
        .map(|cbk| cbk.full_name == full_name)
        .all(|v| v == false)
    {
...
pub extern "C" fn pxs_object_addfunc(
    ...
    add_callback_to_object(
        object_borrow,
        name_borrow,
        callback,
        ObjectFlags::UsesId as u8,
    );
}
...
pub extern "C" fn pxs_objectcall(
    ...
    if runtime_id.is_err() {
        return pxs_Var::incorrect_types_ep(
            vec![pxs_VarType::pxs_Int64, pxs_VarType::pxs_UInt64],
            runtime_borrow.tag,
        )
        .into_raw();
    }
...
pub extern "C" fn pxs_execobject(object: pxs_VarT, local: pxs_VarT) -> pxs_VarT {
    ...
    if !scope.is_map() && !scope.is_null() {
        return pxs_Var::incorrect_types_ep(
            vec![pxs_VarType::pxs_Map, pxs_VarType::pxs_Null],
            scope.tag,
        )
        .into_raw();
    }
...
pub extern "C" fn pxs_core_initall() {
    ...
    with_feature!("pxs_mem", {
        modules |= pxs_core::pxs_ModuleFlag::pxs_MEM as u8;
    });
    with_feature!("pxs_pxs", {
        modules |= pxs_core::pxs_ModuleFlag::pxs_PXS as u8;
    });
    with_feature!("pxs_fs", {
        modules |= pxs_core::pxs_ModuleFlag::pxs_FS as u8;
    });
    with_feature!("pxs_shell", {
        modules |= pxs_core::pxs_ModuleFlag::pxs_SHELL as u8;
    });
    with_feature!("pxs_zip", {
        modules |= pxs_core::pxs_ModuleFlag::pxs_ZIP as u8;
    });
    with_feature!("pxs_http", {
        modules |= pxs_core::pxs_ModuleFlag::pxs_HTTP as u8;
    });

    pxs_core_init(modules);
}
