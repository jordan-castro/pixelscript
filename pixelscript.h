...
extern void curl_easy_cleanup(CURL_PTR curl);

extern void curl_slist_free_all(struct curl_slist *list);

extern enum CURLcode curl_global_init(long flags);

extern CURL_PTR curl_easy_init(void);

extern void curl_easy_reset(CURL_PTR curl);

extern enum CURLcode curl_easy_setopt(CURL_PTR curl, int32_t option, ...);

extern struct curl_slist *curl_slist_append(struct curl_slist *curl_slist, const char *data);

extern enum CURLcode curl_easy_perform(CURL_PTR curl);

extern const char *curl_easy_strerror(enum CURLcode error);

extern enum CURLcode curl_easy_getinfo(CURL_PTR curl, int32_t info, ...);
